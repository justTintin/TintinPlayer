...
public class MediaGroup extends Media
{
    ...
    {
        super(media.getLocation(), media.getTime(), media.getLength(),
                Media.TYPE_GROUP, BitmapUtil.getPictureFromCache(media),
                media.getTitle(), media.getArtist(), media.getGenre(),
                media.getAlbum(), media.getWidth(), media.getHeight(),
                media.getArtworkURL(), media.getAudioTrack(),
                media.getSpuTrack());
        mMedia = media;
    ...
    private static void insertInto(ArrayList<MediaGroup> groups, Media media)
    {
        for (MediaGroup mediaGroup : groups)
        {
            ...
            // not the same prefix, but close : merge
                    ...
                    && (commonLength < group.length() || mediaGroup.size() == 0)
                    && commonLength > MIN_GROUP_LENGTH)
                mediaGroup.merge(media, group.subSequence(0, commonLength)
                        .toString());
            else
                continue;
