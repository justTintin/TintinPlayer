...
public class MediaDatabase
{
    ...
    private class DatabaseHelper extends SQLiteOpenHelper
    {
        ...
        public SQLiteDatabase getWritableDatabase()
        {
            ...
            catch (SQLiteException e)
            {
                try
                {
                    db = SQLiteDatabase.openOrCreateDatabase(VLCApplication.getAppContext()
    ...
    public String[] getPlaylists()
    {
        ...
        cursor = mDb.query(PLAYLIST_TABLE_NAME,
    ...
    {
        mDb.delete(PLAYLIST_TABLE_NAME,
                PLAYLIST_NAME + "=?",
                new String[] { name });
    }
    ...
    public void removeMediaFromPlaylist(String playlistName, String mediaPath)
    {
        mDb.delete(PLAYLIST_MEDIA_TABLE_NAME, PLAYLIST_MEDIA_PLAYLISTNAME
                + "=? " + PLAYLIST_MEDIA_MEDIAPATH + "=?", new String[] {
    ...
    public synchronized Media getMedia(String location)
    {
        ...
        if (cursor.moveToFirst())
        {
            media = new Media(location,
                    ...
                    cursor.getInt(8), cursor.getString(9), cursor.getInt(10),
                    cursor.getInt(11));
        }
        cursor.close();
        return media;
    ...
    public synchronized Bitmap getPicture(Context context, String location)
    {
        ...
        if (cursor.moveToFirst())
        {
            ...
            if (blob != null && blob.length > 1 && blob.length < 500000)
            {
                try
                {
                    picture = BitmapFactory.decodeByteArray(blob,
                            0,
                            blob.length);
                }
    ...
    public static void setPicture(Media m, Bitmap p)
    {
        ...
        try
        {
            getInstance().updateMedia(m.getLocation(),
