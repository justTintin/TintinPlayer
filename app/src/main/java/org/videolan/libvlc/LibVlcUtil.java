...
{
    public final static String TAG = "VLC/LibVLC/VLCUtil";
    ...
    public static boolean hasCompatibleCPU(Context context)
    {
        ...
        { // CPU_ABI2 since 2.2
            try
            {
                CPU_ABI2 = (String) Build.class.getDeclaredField("CPU_ABI2")
                        .get(null);
            }
            catch (Exception e)
            {
        ...
        Log.i(TAG, "machine = "
                + (elf.e_machine == EM_ARM ? "arm"
                        : elf.e_machine == EM_386 ? "x86" : "mips"));
        ...
        if (elf.e_machine == EM_ARM && elf.att_arch.startsWith("v7")
                && !hasArmV7)
        {
    ...
    private static String[] CPU_archs = { "*Pre-v4", "*v4", "*v4T", "v5T",
            "v5TE", "v5TEJ", "v6", "v6KZ", "v6T2", "v6K", "v7", "*v6-M",
            "*v6S-M", "*v7E-M", "*v8" };
    ...
    private static boolean readHeader(RandomAccessFile in, ElfData elf)
            throws IOException
    {
        // http://www.sco.com/developers/gabi/1998-04-29/ch4.eheader.html
        byte[] bytes = new byte[ELF_HEADER_SIZE];
        in.readFully(bytes);
        if (bytes[0] != 127 || bytes[1] != 'E' || bytes[2] != 'L'
                || bytes[3] != 'F' || bytes[4] != 1)
