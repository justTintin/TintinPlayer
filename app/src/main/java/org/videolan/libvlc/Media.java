...
import org.videolan.vlc.util.VLCUtil;
...
public class Media implements Comparable<Media>
{
    ...
    static
    {
        ...
        for (String item : folder_blacklist)
            FOLDER_BLACKLIST.add(android.os.Environment.getExternalStorageDirectory()
                    .getPath()
                    + item);
    }
     ...
     *
     * @see VLCUtil#getValue(String, int)
     *
    ...
    private static String getValueWrapper(String string, UnknownStringType type)
    {
        ...
        try
        {
            ...
            Class<?> utilClass = Class.forName("org.videolan.vlc.VLCUtil");

            Integer value;
            switch (type)
            {
                case Album:
                    value = (Integer) stringClass.getField("unknown_album")
                            .get(null);
                break;
                case Genre:
                    value = (Integer) stringClass.getField("unknown_genre")
                            .get(null);
                break;
                case Artist:
                default:
                    value = (Integer) stringClass.getField("unknown_artist")
                            .get(null);
                break;
            }
            ...
            Method getValueMethod = utilClass.getDeclaredMethod("getValue",
                    String.class,
                    Integer.TYPE);
            // VLCUtil.getValue(string, R.string.xxx);
    ...
    {
        return mTitle.toUpperCase(Locale.getDefault())
                .compareTo(another.getTitle().toUpperCase(Locale.getDefault()));
    }
